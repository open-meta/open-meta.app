...
citesFilter <- function(DB, TABLE, tableID, WHERE) {       # these aren't actually used in this filter, but they are passed...
   ...
   if(f$author!="")  { WHERE$author=c("author", "LIKE", paste0("%", f$author, "%"))}
   if(f$year!="")    { WHERE$year=c("Y", "LIKE", paste0("%", f$year, "%"))}
   if(f$journal!="") { WHERE$journal=c("journal", "LIKE", paste0("%", f$journal, "%"))}
   if(extract) {
...
### Special SQL Request to OR title and abstact
   ...
   r <- r %>% arrange(Y, author) %>% select(catalogID, reviewBest)      # We sort twice, once here
###                                                                     #   for entire vector of filteredIDs
