   ...
   output$uiMeat <- renderUI({c(rv$menuActive, rv$limn); isolate({
      if(rv$limn && S$P$Msg=="") {
         if(S$P$Modify) {                               # Modification allowed?
            return(tagList(
               bs4("md", id="sub", n=1:7, active=rv$menuActive, text=c("Dashboard", "PRISMA diagram", "Trial Sequential Analysis", "Forest Plot", "Bias Plots", "Subgroup Analysis", "Meta-Regression")),
