   ...
   output$uiMeat <- renderUI({c(rv$menuActive, rv$limn); isolate({
      if(rv$limn && S$P$Msg=="") {
         if(S$P$Modify) {                               # Modification allowed?
            return(tagList(
               bs4("md", id="sub", n=1:5, active=rv$menuActive, text=c("New Trial", "Trial List", "New Outcome", "Outcome List", "New Arm")),
               bs4("r", bs4("ca", "More to come..."))
            ))
         } else {                                       # Otherwise view only
            return(tagList(
               bs4("md", id="sub", n=1:2, active=rv$menuActive, text=c("Trial List", "Outcome List")),
               bs4("r", bs4("ca", "More to come..."))
            ))
         }
      }
